%Captures and saves a Z stack using Alfred. Z position is moved using the Asi Piezeo Z stage.
...
function [stack maxvalue]=captureStackAlfred(filename,thisZ, zinfo, offset, EM, E,height,width)
global mmc;
nSlices=zinfo(1);
sliceInterval=zinfo(2);
anyZ=zinfo(4);
stack=zeros(height,width,nSlices);
startPos=mmc.getPosition('ZStage');%starting position
maxvalue=0;
