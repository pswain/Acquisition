...
switch obj.Name
    case 'Batman'
        if nargin>1%Verbose input from micromanager config for this channel has been supplied
            LEDplace=strfind(verbose,'DTOL-Switch:State=');
            matVerbose=char(verbose);%Convert to MATLAB string
            DTolState=matVerbose(LEDplace+18);
            switch(str2num(DTolState))
                case 1
                    LED='DAC0';
