function details=enterDetails(previousDetails)
str = sprintf(previousDetails);
default={str};
input = inputdlg('Enter experimental details here:','Enter experimental details',10,default);
...
input=input{1}(:,:);
    ...
    input=cellstr('No details entered');
end
%make sure each line has max 90 characters.
%Not coded yet
%use linewrap function below for that.

s=size(input);
details='';
for n=1:s(1)
details=strcat(details,input(n,:));
end
