...
%PUT THESE LINES BACK IN BUT MANAGE PFS
...
%pumpOn=acqData.flow{3};
%flowing=acqData.flow{pumpOn};
...
for t=1:numTimepoints%start of timepoint loop.
   ...
   %if numPositions>1%Is point visiting required?
   maxgroups=zeros(numGroups,numChannels);
        ...
        for pos=1:numPositions
            ...
            if acqData.z(3)==1 && single==0
               ...
               if acqData.z(4)~=0% == 1 if any channel does z sectioning.
                   ...
                   %If any channel at this point does z sectioning - visit
                   %the z position at the top of the stack.
%                    if anyZThisPos
%                       startingZ=visitZ(logfile,acqData.z,acqData.logtext,acqData.points(pos,:));
%                    else
%                       zInfo=acqData.z;
%                       zInfo(4)=0;%anyz set to zero.
%                       startingZ= visitZ(logfile,zInfo,acqData.logtext,acqData.points(pos,:));
%                    end
               else
